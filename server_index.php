...
function checkQuizeeAnsweredToday($rcp_id, $mobile, $email) {
    $sql = "SELECT COUNT(*) AS count FROM Quizee_Details where receipt_no = '$rcp_id' AND Mobile ='$mobile' AND Email = '$email' AND DATE(date_appeared) = CURDATE()";
    // echo"SQL".$sql;
    $db = getDB();
    $stmt = $db->prepare ( $sql );
    $stmt->execute ();
...
//        $date = date('Y-m-d H:i:s');
        ...
        if (!empty ($data ['rcp_id']) && !empty ($data ['user_id']) && !empty ($data ['opt_1']) && !empty ($data ['opt_2']) && !empty ($data ['opt_3']) && !empty ($data ['opt_4']) && !empty ($data ['opt_5'])) {
            ...
            $sql2 = "SELECT Quizee_Details.id,Quizee_Details.Email, Quizee_Details.Mobile FROM Quizee_Details INNER JOIN ACStudents 
ON Quizee_Details.id = ACStudents.quizee_user_id WHERE ACStudents.id= '$quizee_id'";
            ...
            $quizee_answered = checkQuizeeAnsweredToday($data ['rcp_id'], $mobile, $email);
...
function getQuizeePdf() {
    ...
    if ($json != null) {
        ...
        if (! empty ( $data ['id'] ))
        {
            ...
            $email = $registration_details_json2[0]['Email'];
            $mobile = $registration_details_json2[0]['Mobile'];
            $sql3 = "SELECT quizee_user_id FROM ACStudents WHERE email = '$email' AND mobile = '$mobile'";
            ...
            $sql = "SELECT Quizee_Details.id, QuizeeDocument.url FROM Quizee_Details INNER JOIN QuizeeDocument ON Quizee_Details.id = QuizeeDocument.user_id WHERE QuizeeDocument.user_id = '$quizee_user_id'";
