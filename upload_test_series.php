...
if(isset($_FILES['pdf']['name']) and isset($_POST['name'])and isset($_POST['class_name'])){
    ...
    try{
        ...
        $result = mysqli_query($con,"SELECT * FROM AllTestSeries WHERE code ='$code' ");

        if( mysqli_num_rows($result) > 0) {
            mysqli_query($con,"UPDATE AllTestSeries SET url = '$file_url' WHERE code = '$code' ");
        }
        else{
            //saving the file
            date_default_timezone_set('Canada/Pacific');
            $today = date('m/d/Y h:i:s a', time());
            $sql = "INSERT INTO AllTestSeries( url, test_series_name, class, code) VALUES ('$file_url','$name','$class_name','$code');";
...
function getFileName(){
    $con = mysqli_connect(DB_HOST,DB_USERNAME,DB_PASSWORD,DB_NAME) or die('Unable to Connect...');
    //$con = getDB();
    $sql = "SELECT max(id) as id FROM AllTestSeries";
